...
int mk_createSock(const char* holePunchingID)
{
    ...
    //1. create peer
    //2. 把holePunchingID vs peer, 可能還會加入arrray裡
    ...
    for (i=0; i<(int)PJ_ARRAY_SIZE(g.peer); ++i)
    {
        ...
        //        if (o.stun_server) {
        server = pj_str("stun.counterpath.com");
        port = PJ_STUN_PORT;
        //        } else {
...
//
////        if (o.stun_server) {
//        server = pj_str("stun.counterpath.com");//o.stun_server);
//        port = PJ_STUN_PORT;
////        } else {
////            server = pj_str(o.srv_addr);
////            port = (pj_uint16_t)(o.srv_port?atoi(o.srv_port):PJ_STUN_PORT);
...
//            char  relay_addr[80];
//            strcpy(relay_addr, "192.168.11.2:12345");
            
//            pj_sockaddr_t *dstAddr;
            pj_sockaddr dst2A;
//            pj_sockaddr_cp(&bind_addr, addr);
            pj_uint16_t port=(pj_uint16_t)12345;
//            pj_sockaddr_set_port(&dst2A, port);
            pj_str_t ns = pj_str("192.168.11.2");
            pj_sockaddr_init(pj_AF_INET(), &dst2A, &ns, port);
//            pj_sockaddr_set_str_addr(pj_AF_INET(), &dst2A, &ns);

//            pj_sockaddr_t dst1;
//            pj_sockaddr_cp(&dst1, &dst2A);
